...
# 磁気値を取得
def getMag(magCoefficient14, magCoefficient16):
        global MAG_ACCESS
        if MAG_ACCESS == False:
            # 磁気センサへのアクセスが有効になっていない場合は例外
...
def cal_roll(acc):
    ...
    accy = float(acc[1])
    accz = float(acc[2])
    roll = degrees(atan2(accy, sqrt(accz*accz+(accx*accx))))
    return roll
...
def cal_pitch(acc):
    ...
    accy = float(acc[1])
    accz = float(acc[2])
    pitch = degrees(atan2(accx, sqrt(accz*accz+(accy*accy))))
    return pitch
...
#     accx = float(acc[0])
#     accy = float(acc[1])
#     yaw = atan2(accx, accy) * 180 / pi
#     return yaw

#モータ制御、エックス軸、てきとー
def con_roll(roll, pulse):
    if roll > 0:
        pulse += 10
    elif roll < 0:
        pulse -= 10
