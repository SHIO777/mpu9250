...
"""ジャイロセンサの値を取得"""
file_name = "accel_uncalib_{0:%Y%m%d-%H%M%S}".format(datetime.datetime.now())

fmt_name = "/home/pi/data/{}.csv".format(file_name)
header = "yyyy-mm-dd hh:mm:ss.mmmmmm,x [g],y [g],z [g]"
f = open(fmt_name, "w")
f.write(header+"\n")
for i in range(500):
    ...
    value = "%s,%6.3f,%6.3f,%6.3f" % (date, accel[0], accel[1], accel[2])
    f.write(value+"\n")
