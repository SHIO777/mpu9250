...
# accel_cal()でオフセット取得するので，ここではキャリブレーションしない
sensor.set_accel_range(False)
sensor.set_gyro_range(False)
    ...
    while True:
        ...
        while True:
            try:
                ...
                mx,my,mz = sensor.get_mag() # read and convert AK8963 magnetometer data
...
#
        ...
        ###################################
        #
        data = np.array([sensor.get_accel() for ii in range(0,cal_size)]) # new values
        #
        ###################################
